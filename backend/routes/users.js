...
// POST /api/users/admin/remove-skin
// Body: { targetUid: string, skinId: string }
router.post('/admin/remove-skin', requireAuth, requireAdmin, async (req, res) => {
  const { targetUid, skinId } = req.body;
  if (!targetUid || !skinId) return res.status(400).json({ error: 'targetUid and skinId required' });
  if (skinId === 'agent') return res.status(400).json({ error: 'Cannot remove the default skin' });
  try {
    const user = await getUserById(targetUid);
    if (!user) return res.status(404).json({ error: 'User not found' });
    ...
    return res.status(500).json({ error: 'Failed to remove skin' });
  }
});
