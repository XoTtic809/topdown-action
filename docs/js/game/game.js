...
function getActiveSkinColor() {
  ...
  return getSkinFlatColor(activeSkin, _frameNow);
}
...
class Player {
 ...
 // Gradient body with glow (falls back to flat color in perfMode)
      ctx.fillStyle = getSkinFill(ctx, activeSkin, this.x, this.y, this.r, _frameNow);
      ctx.shadowBlur = 20;
      ctx.shadowColor = getSkinGlow(activeSkin, _frameNow);
      ctx.beginPath();
      ctx.arc(this.x, this.y, this.r, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
...
function applyRichSkinPreview(el, skinId, fallbackColor) {
  return applySkinPreview(el, skinId, fallbackColor);
}
...
function loop(time) {
 // Skip one frame after tab refocus to avoid huge dt spikes
  ...
  if (player) window.__gameDebug = { player, enemies, score, coins, wave, bullets, particles, powerups };
