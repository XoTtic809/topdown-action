// mp-hook.js — v5
// Fixes: player visibility (initialize dummy player), dash (space key → server),
//        pause overlay (ESC), full skin rendering matching single-player.
...
(function () {
  ...
  function drawMpPlayer(p) {
    // Ensure a player object exists for rendering — create one if needed
    if (!player) {
      try { player = new Player(canvas.width / 2, canvas.height / 2); }
      catch(e) { return; }
    }

    // --- Dead / awaiting revive ---
    if (!p.alive) {
      ctx.save();
  ...
  function drawMpEnemy(e) {
    ...
    ctx.beginPath(); ctx.arc(e.x, e.y, e.radius ?? e.r, 0, Math.PI * 2); ctx.fill();
    ctx.shadowBlur = 0;
    ...
    if (type === 'shooter') { ctx.fillStyle = 'rgba(255,255,255,0.9)'; ctx.font = 'bold 12px Arial'; ctx.fillText('!', e.x, e.y); }
    else if (type === 'miniboss') { ctx.fillStyle = 'rgba(255,255,255,0.9)'; ctx.font = 'bold 12px Arial'; ctx.fillText('⚠', e.x, e.y); }
    else if (type === 'enforcer') { ctx.fillStyle = e.dashChargeTime > 0 ? '#ff4444' : 'rgba(255,255,255,0.9)'; ctx.font = 'bold 14px Arial'; ctx.fillText(e.dashChargeTime > 0 ? '⚡' : '⬥', e.x, e.y); }
    ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
  }
  ...
  function mpRenderFrame() {
    ...
    if (shaking) { ctx.save(); ctx.translate((Math.random()-0.5)*screenShakeAmt*25,(Math.random()-0.5)*screenShakeAmt*25); }
    ...
    if (!s) {
      ...
      if(shaking)ctx.restore(); return;
    }

    // Trail particles (behind players)
    if(typeof particles!=='undefined') particles.forEach(p=>{if(p.isTrail&&typeof p.draw==='function')p.draw();});
    ...
    if(s.bullets?.length){
      ...
      s.bullets.forEach(b=>{ctx.fillStyle='#ffe66b';ctx.beginPath();ctx.arc(b.x,b.y,b.r??5,0,Math.PI*2);ctx.fill();});
      ctx.shadowBlur=0;
    }
    if(s.enemyBullets?.length){
      ctx.shadowBlur=10; ctx.shadowColor='#ff4757';
      s.enemyBullets.forEach(eb=>{ctx.fillStyle='#ff4757';ctx.beginPath();ctx.arc(eb.x,eb.y,eb.r??7,0,Math.PI*2);ctx.fill();});
      ctx.shadowBlur=0;
    }
    ...
    s.powerups?.forEach(pu=>{
      ...
      ctx.beginPath(); ctx.arc(pu.x,pu.y+bob,pu.r??12,0,Math.PI*2); ctx.fillStyle=pu.color||'#ffffff'; ctx.fill();
      ctx.shadowBlur=0; ctx.fillStyle='#fff'; ctx.font='bold 14px Arial';
      ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(pu.symbol||'✦',pu.x,pu.y+bob);
    ...
    if(!s.boss&&s.waveClearTimer<=0&&s.bossCountdownTimer<=0){
      const wave=s.wave||1;
      const remaining=Math.max(0,(wave*5+12)-(s.enemiesKilledThisWave||0));
      ctx.save();
      ...
      ctx.font='700 18px Inter, system-ui';ctx.fillStyle=cC;ctx.shadowBlur=6;ctx.shadowColor=cC;ctx.fillText(remaining,iX+18,iY+9);ctx.shadowBlur=0;
      ctx.restore();
    }
  ...
  window.addEventListener('load', () => {
    setTimeout(() => {
      ...
      // Capture-phase keydown — fires before game.js handler
      window.addEventListener('keydown', function mpKeydown(e) {
        if (!e.key) return;
        if (typeof mpGameActive === 'undefined' || !mpGameActive) return;
        if (e.key === ' ') {
          e.preventDefault();
          window._mpDashPending = true;  // consumed by next mpSendInput tick
        }
        if (e.key === 'Escape') {
          e.preventDefault();
          mpTogglePause();
      ...
      console.log('[MP] v5 — loop + keys wrapped');
    }, 200);
  });
  ...
  window.addEventListener('load', () => {
    setTimeout(() => {
      ...
      console.log('[MP] v5 — mpSendInput patched');
    }, 500);
  });

  // ── Wire pause overlay buttons for MP ─────────────────────
  window.addEventListener('load', () => {
    setTimeout(() => {
  ...
  window.addEventListener('load', () => {
    setTimeout(() => {
      ...
      window.startMpGame = function (wave) {
        ...
        // Always create a fresh player instance so drawMpPlayer can call player.draw()
        try { player = new Player(canvas.width / 2, canvas.height / 2); } catch(e) {}
        if (_origStart) _origStart(wave);
        requestAnimationFrame(window.loop);
        console.log('[MP] v5 — game started, skins + dash + pause active');
      };
      ...
      window.mpReturnToMenu = function () {
        ...
        prevState = null; currentState = null; mpPaused = false;
  ...
  canvas?.addEventListener('touchstart', e => {
    ...
    if (typeof mouse !== 'undefined') { mouse.x = (t.clientX-r.left)*(canvas.width/r.width); mouse.y = (t.clientY-r.top)*(canvas.height/r.height); }
  }, { passive: true });
  ...
  console.log('✅ mp-hook.js v5 loaded');
})();