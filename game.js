...
function createCreatorKillEffect(x, y) {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    particles.push(new Particle(x, y, a, spd,
      igniteColors[Math.floor(Math.random() * igniteColors.length)], 0.25 + Math.random() * 0.15));
  }
  // collapse spark flash
  setTimeout(() => {
    for (let i = 0; i < 8; i++) {
      const p = new Particle(x + (Math.random() - 0.5) * 12, y + (Math.random() - 0.5) * 12,
...
function initShopUI() {
 ...
 // Also initialize battle pass tab
  ...
  const regularSkinsOrdered = SKINS
    .filter(s => !s.crateOnly && s.battlePassTier === undefined && s.price !== -1)
...
if (gameSettings.screenShake) screenShakeAmt = 1;
 ...
 // Submit score immediately (don't wait for visual delay)
      if (currentUser && !isGuest && typeof acSubmitScore === 'function') {
        ...
        saveUserDataToFirebase('critical');  // Critical: game over, must save immediately
      }
