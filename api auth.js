...
function _applyUserData(data) {
  ...
  // Battle pass
  if (typeof battlePassData !== 'undefined' && data.battlePassData) {
    const bp = data.battlePassData;
  ...
  // Crate inventory
  if (typeof battlePassData !== 'undefined' && data.crateInventory) {
    battlePassData.crateInventory = data.crateInventory;
  } else if (typeof battlePassData !== 'undefined' && !battlePassData.crateInventory) {
    battlePassData.crateInventory = {
...
window.addEventListener('load', async () => {
  ...
  try {
    ...
    if (data.error) { clearToken(); setTimeout(() => showAuthModal(), 300); return; }

    // Load battle pass + crates from server
    try {
      const bp = await apiGet('/battlepass/me');
      if (!bp.error) {
        data.battlePassData = bp.battlePassData;
        data.crateInventory = bp.crateInventory;
      }
    } catch (e) { /* non-fatal */ }
...
async function _saveBattlePass() {
  ...
  try {
    await apiPost('/battlepass/save', {
      battlePassData: {
        season:            battlePassData.season           || 1,
        currentXP:         battlePassData.currentXP        || 0,
        currentTier:       battlePassData.currentTier      || 0,
        isPremium:         battlePassData.isPremium        || false,
        claimedRewards:    battlePassData.claimedRewards   || { free: [], premium: [] },
        ...
        activeTrail:       battlePassData.activeTrail      || null,
        activeDeathEffect: battlePassData.activeDeathEffect|| null,
        activeTitle:       battlePassData.activeTitle      || null,
        activeBadge:       battlePassData.activeBadge      || null,
      },
...
async function executeSave() {
  ...
  try {
    ...
    if (!hasChanges && !forceNextSave) { pendingSave = false; return; }
    ...
    const result = await apiPost('/auth/progress', {
      highScore: high, totalCoins: playerCoins, currentXp,
    });
...
async function updateLeaderboardsIfNeeded(){ /* no-op */ }

async function fetchLeaderboard(type = 'allTime') {
  try {
    if (type === 'coins') {
      return (await apiGet('/leaderboard/coins?limit=100')).map(r => ({
        userId: r.uid, username: r.username, coins: r.total_coins,
      }));
    } else if (type === 'level') {
      return (await apiGet('/leaderboard/levels?limit=100')).map(r => ({
        userId: r.uid, username: r.username, xp: r.current_xp,
        level: typeof calculateTrueLevel === 'function' ? calculateTrueLevel(r.current_xp) : 0,
      }));
    } else {
      return (await apiGet('/leaderboard/scores?limit=100')).map(r => ({
        userId: r.uid, username: r.username, score: r.high_score,
      }));
    }
  } catch (err) { console.error('[LB] fetch failed:', err); return []; }
}
...
async function updateChampionSkins() {
  ...
  try {
    ...
    if (rank > 3) {
      let changed = false;
      ['gold-champion','silver-champion','bronze-champion'].forEach(skin => {
        const idx = ownedSkins.indexOf(skin);
        if (idx > -1) { ownedSkins.splice(idx,1); if (activeSkin===skin) activeSkin='agent'; changed=true; }
      });
      if (changed) await saveUserDataToFirebase('critical');
    }
  } catch (err) { console.error('[Champion] failed:', err); }
}
...
async function loadUserDataFromFirebase() {
  try {
    ...
    if (!data.error) {
      const bp = await apiGet('/battlepass/me');
      if (!bp.error) { data.battlePassData = bp.battlePassData; data.crateInventory = bp.crateInventory; }
      _applyUserData(data);
    }
  } catch (e) { /* ignore */ }
}
...
async function fetchBannedUsers() {
  if (!isAdmin) return [];
  try {
    return (await apiGet('/users/admin/list') || [])
      .filter(u => u.is_banned)
      .map(u => ({ userId: u.uid, reason: u.ban_reason || '', username: u.username }));
  } catch (err) { return []; }
}
  ...
  banned.forEach(entry => {
    ...
    el.innerHTML = `
      <div>
        ...
        <div style="font-size:10px;color:var(--muted);">${entry.reason || 'No reason'}</div>
...
async function handleUnban(userId) {
  ...
  showAdminMessage(r.success ? 'User unbanned' : 'Error: '+r.error, !r.success);
  if (r.success) displayBannedUsers();
}

async function logAdminAction() { /* logged server-side automatically */ }

async function fetchAllUsers() {
  if (!isAdmin) return [];
  try {
    return (await apiGet('/users/admin/list') || []).map(u => ({
      userId: u.uid, username: u.username, email: u.email,
      highScore: u.high_score, totalCoins: u.total_coins,
      currentXp: u.current_xp, is_banned: u.is_banned,
      createdAt: u.created_at, gamesPlayed: 0,
    }));
  } catch (err) { return []; }
}
  ...
  users.forEach(user => {
    ...
    el.innerHTML = `
      <div class="admin-user-info">
        <div class="admin-user-name">${user.username||'Unknown'}${user.is_banned?'<span style="color:#ff6b7a;font-size:10px;margin-left:4px;">BANNED</span>':''}</div>
        <div class="admin-user-meta">${user.email||''} · Joined: ${joined}</div>
        <div class="admin-user-meta">🏆 ${(user.highScore||0).toLocaleString()} · 🪙 ${(user.totalCoins||0).toLocaleString()}</div>
...
async function deleteLeaderboardScore(u,n)      { return resetUserHighScore(u,n); }
async function deleteCoinsLeaderboardScore()    { return { success: true }; }
async function deleteLevelLeaderboardScore()    { return { success: true }; }

async function fetchActivityLogs() {
  if (!isAdmin) return [];
  try {
    return (await apiGet('/users/admin/logs') || []).map(l => ({
      id: l.id, action: l.action, adminId: l.admin_id, adminName: l.admin_name,
      targetUserId: l.target_uid, targetUsername: l.target_uid, details: l.details,
...
async function displayActivityLogs() {
  ...
  const labels = {
    BAN_USER:'🚫 Banned', UNBAN_USER:'✅ Unbanned', RESET_SCORE:'🔄 Reset score',
    GRANT_SKIN:'🎁 Gave skin', GRANT_COINS:'🪙 Gave coins',
    BLOCK_SKIN_TRADE:'⛔ Blocked skin trade', UNBLOCK_SKIN_TRADE:'✅ Unblocked skin trade',
  };
  listEl.innerHTML = '';
  logs.forEach(log => {
    const el = document.createElement('div');
    ...
    const label  = labels[log.action] || log.action || 'Action';
...
async function displayScoresAdmin() {
  ...
  listEl.innerHTML = '<div class="loading-spinner">Loading...</div>';
  const lb = await fetchLeaderboard('allTime');
  if (lb.length === 0) { listEl.innerHTML = '<div class="loading-spinner">No scores yet</div>'; return; }
  listEl.innerHTML = '';
  lb.forEach((entry,i) => {
    const el = document.createElement('div');
...
async function displayCoinsScoresAdmin() {
  ...
  if (lb.length === 0) { listEl.innerHTML = '<div class="loading-spinner">No entries yet</div>'; return; }
  listEl.innerHTML = '';
  lb.forEach((entry,i) => {
    const el = document.createElement('div');
    el.className = 'admin-score-entry';
...
async function displayPlatformStats() {
  ...
  try {
    ...
    set('activeToday', '—');
    set('totalGames',  '—');
    set('avgScore', users.length > 0
      ? Math.round(users.reduce((s,u) => s+(u.high_score||0), 0) / users.length) : 0);
    ...
    if (topEl) {
      ...
      lb.slice(0,10).forEach((e,i) => {
        ...
        row.innerHTML = `<div class="admin-score-rank">${i+1}</div><div class="admin-score-info"><div class="admin-score-name">${e.username||'?'}</div><div class="admin-score-val">🏆 ${(e.score||0).toLocaleString()}</div></div>`;
        topEl.appendChild(row);
      });
    }
  } catch (err) { fields.forEach(id => { const el=document.getElementById(id); if(el) el.textContent='ERR'; }); }
}

// ── Skin management ──
function adminInitSkinGiveDropdown() {
  const select = document.getElementById('skinGiveSelect');
  if (!select || typeof SKINS === 'undefined' || select.options.length > 1) return;
...
async function adminLookupUserSkins() {
  ...
  try {
    ...
    let html = `<div style="padding:8px 0;color:var(--muted);font-size:11px;border-bottom:1px solid rgba(255,255,255,0.05);margin-bottom:6px;">
      <strong style="color:#fff;">${data.username}</strong> — ${owned.length} skins, active: <strong style="color:#6bff7b;">${active}</strong></div>`;
    owned.forEach(skinId => {
      const info = typeof SKINS !== 'undefined' ? SKINS.find(s => s.id === skinId) : null;
      html += `<div style="padding:4px 0;font-size:12px;">${skinId===active?'⚡ ':''}${info?.name||skinId} <span style="color:var(--muted);font-size:10px;">(${skinId})</span></div>`;
    });
    resultEl.innerHTML = html;
  } catch (err) { resultEl.innerHTML = `<div style="color:#ff6b7a;padding:12px;">Error: ${err.message}</div>`; }
}
...
async function adminGiveSkin() {
  ...
  const info = typeof SKINS !== 'undefined' ? SKINS.find(s => s.id === skinId) : null;
  showAdminMessage(`Gave "${info?.name||skinId}" successfully`);
}

async function adminRemoveSkin() { showAdminMessage('Skin removal: use Railway DB directly', true); }
async function adminLoadUserSkinsForRemoval() { return adminLookupUserSkins(); }
...
async function adminLoadTradeRestrictions() {
  ...
  try {
    ...
    if (!rows.length) { listEl.innerHTML = '<div class="loading-spinner">No restrictions set.</div>'; return; }
    listEl.innerHTML = '';
    rows.forEach(r => {
      const el = document.createElement('div');
      el.className = 'admin-log-entry';
      el.innerHTML = `
        <div class="admin-log-action" style="font-family:monospace;font-size:11px;">
          ${r.skin_id} <span style="color:var(--danger);margin-left:6px;">⛔ BLOCKED</span>
        </div>
        <div class="admin-log-meta">Reason: ${r.reason||'None'} · by ${r.added_by||'?'}</div>
        <button class="admin-action-btn success" onclick="adminUnblockSkinTrade('${r.skin_id}')"
                style="margin-top:4px;padding:4px 10px;font-size:10px;">✅ UNBLOCK</button>`;
      listEl.appendChild(el);
    });
  } catch (err) { listEl.innerHTML = `<div style="color:var(--danger);padding:8px;">Error: ${err.message}</div>`; }
}
...
async function adminUnblockSkinTrade(skinId) {
  ...
  if (!confirm(`Unblock trading for "${skinId}"?`)) return;
  const data = await apiDelete(`/trade-restrictions/admin/${skinId}`);
  if (data.error) { showAdminMessage('Error: '+data.error, true); return; }
  showAdminMessage(`Trade restriction removed for "${skinId}".`);
  adminLoadTradeRestrictions();
}

// ── Skin/crate price editors (in-memory only) ──
function adminInitSkinPriceDropdown() {
  ...
  SKINS.filter(s => s.price > 0).forEach(skin => {
    const opt = document.createElement('option');
    opt.value = skin.id; opt.textContent = `${skin.name} — 🪙 ${skin.price}`;
    select.appendChild(opt);
  });
}
function adminSkinPriceSelectChange() {
  ...
  const input=document.getElementById('skinPriceEditValue'); if(input) input.value=skin.price;
  const prev=document.getElementById('skinPriceCurrentVal'); if(prev) prev.textContent=`Current: 🪙 ${skin.price}`;
}
async function adminSaveSkinPrice() {
  const skinId=document.getElementById('skinPriceEditSelect')?.value;
  const newPrice=parseInt(document.getElementById('skinPriceEditValue')?.value)||0;
  if(!skinId){showAdminMessage('Select a skin',true);return;}
  if(newPrice<50||newPrice>100000){showAdminMessage('Price 50–100,000',true);return;}
  const skin=typeof SKINS!=='undefined'?SKINS.find(s=>s.id===skinId):null;
  if(!skin){showAdminMessage('Skin not found',true);return;}
  const old=skin.price; skin.price=newPrice;
  showAdminMessage(`"${skin.name}" 🪙 ${old} → 🪙 ${newPrice} (session only — resets on reload)`);
...
function adminInitCratePriceDropdown() {
  ...
  if(!select||typeof CRATES==='undefined'||select.options.length>1) return;
  select.innerHTML='<option value="">-- Select a crate --</option>';
  CRATES.forEach(c=>{const opt=document.createElement('option');opt.value=c.id;opt.textContent=`${c.name} — 🪙 ${c.price}`;select.appendChild(opt);});
}
function adminCratePriceSelectChange() {
  ...
  const c=CRATES.find(x=>x.id===crateId); if(!c) return;
  const input=document.getElementById('cratePriceEditValue'); if(input) input.value=c.price;
  const prev=document.getElementById('cratePriceCurrentVal'); if(prev) prev.textContent=`Current: 🪙 ${c.price}`;
}
async function adminSaveCratePrice() {
  ...
  if(newPrice<100||newPrice>100000){showAdminMessage('Price 100–100,000',true);return;}
  const c=typeof CRATES!=='undefined'?CRATES.find(x=>x.id===crateId):null;
  if(!c){showAdminMessage('Crate not found',true);return;}
  const old=c.price; c.price=newPrice;
  showAdminMessage(`"${c.name}" 🪙 ${old} → 🪙 ${newPrice} (session only)`);
...
async function removeFlaggedScore() { showAdminMessage('Not available in self-hosted mode', true); }
